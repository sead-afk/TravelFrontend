...
export async function loadUserProfile() {
    try {
        ...
        const userResponse = await fetch(`${window.API_BASE_URL}/api/users/profile`, {
            method: 'GET',
            headers: {
        ...
        const bookingResponse = await fetch(`${window.API_BASE_URL}/api/bookings/user/${user.username}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
        ...
        flightBookings.forEach(booking => {
            ...
            fetch(`${window.API_BASE_URL}/api/flights/${flightResource}`)
        ...
        for (const booking of hotelBookings) {
            ...
            const hotelResponse = await fetch(`${window.API_BASE_URL}/api/hotels/${hotelResource}`);
...
async function submitEditBooking(booking, type) {
    try {
        ...
        const response = await fetch(`${window.API_BASE_URL}/api/bookings/${booking.id || booking._id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
...
async function updateUserBalance() {
    try {
        ...
        const response = await fetch(`${window.API_BASE_URL}/api/users/add-funds`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
...
async function loadAvailableHotelsForEdit(currentHotelId, currentRoomId) {
    try {
        const token = localStorage.getItem("jwt");
        const hotelResponse = await fetch(`${window.API_BASE_URL}/api/hotels/`, {
            headers: {"Authorization": `Bearer ${token}`}
        });
        if (!hotelResponse.ok) {
...
async function loadAvailableRoomsForEdit(hotelId, currentRoomId) {
    try {
        const token = localStorage.getItem("jwt");
        const roomResponse = await fetch(`${window.API_BASE_URL}/api/hotels/${hotelId}/rooms`, {
            headers: {"Authorization": `Bearer ${token}`}
        });
        if (!roomResponse.ok) {
...
async function loadAvailableFlightsForEdit(currentFlightId, currentTicketId) {
    try {
        const token = localStorage.getItem("jwt");
        const flightResponse = await fetch(`${window.API_BASE_URL}/api/flights/`, {
            headers: { "Authorization": `Bearer ${token}` }
        });
...
async function loadAvailableSeatsForEdit(flightId, currentTicketId) {
    try {
        const token = localStorage.getItem("jwt");
        const response = await fetch(`${window.API_BASE_URL}/api/flights/${flightId}/tickets`, {
            headers: { "Authorization": `Bearer ${token}` }
        });
        if (!response.ok) {
